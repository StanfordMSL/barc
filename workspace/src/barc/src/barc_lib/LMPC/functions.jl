...
function InitializeParameters(mpcParams::MpcParams,mpcParams_pF::MpcParams,trackCoeff::TrackCoeff,modelParams::ModelParams,
    ...
    mpcParams.Q_term            = 1.0*[200.0,200.0,200.0,200.0]  #m # weights for terminal constraints (LMPC, for xDot,yDot,psiDot,ePsi,eY)
    mpcParams.R                 = [0.5,0.5]                 # put weights on a and d_f
    mpcParams.QderivZ           = 1.0*[0,0,0,0,1.0] #m            # cost matrix for derivative cost of states
    mpcParams.QderivU           = 1.0*[5.0,100.0,0.0]                # cost matrix for derivative cost of inputs
