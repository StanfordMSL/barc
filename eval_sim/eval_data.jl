...
function eval_LMPC(code::AbstractString)
    ...
    subplot(311)
    title("c_Vx")
    plot(t-t0,c_Vx[:,1],"r",t-t0,c_Vx[:,2],"b",t-t0,c_Vx[:,3],"g")
    grid("on")
    subplot(312)
    title("c_Vy")
    plot(t-t0,c_Vy[:,1],"r",t-t0,c_Vy[:,2],"b",t-t0,c_Vy[:,3],"g",t-t0,c_Vy[:,4],"c")
    grid("on")
    subplot(313)
    title("c_Psi")
    plot(t-t0,c_Psi[:,1],"r",t-t0,c_Psi[:,2],"b",t-t0,c_Psi[:,3],"g")
    grid("on")
...
function create_track(w::Float64=0.4,showplot::Bool=false)
